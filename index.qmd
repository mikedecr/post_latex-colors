...
but I worked up a quick exercise when [Jordan Nafa was talking about color-coding different parts of some equations](https://twitter.com/adamjnafa/status/1565164364981620737).

So we will the broader problem of **color-coding an equation** as an example.
...
The color options aren't the real frustration though (you can load in extra colors, whatever, who cares).
The real frustration is the _interface_, which works like this.
...
To be more specific, I want to _map_ certain semantic features of my document's content to specific colors in a consistent way.
We would like the interface to be minimally burdensome, so I don't want to type (or _think_) a lot to get good effects.

Here are our equation and our "semantics".
